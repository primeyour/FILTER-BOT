...
async def save_file(media):
    ...
    try:
        ...
        logger.warning(f'{media.file_name} already saved in database')
        return False, 0
    logger.info(f'{media.file_name} saved to database')
    return True, 1
...
async def get_file_details(query):
    print(f"📂 [DEBUG] Searching Mongo for file_id: {query}")
    filter = {'file_id': query}
    cursor = Media.find(filter)
    filedetails = await cursor.to_list(length=1)
    print(f"📂 [DEBUG] Results: {filedetails}")
    return filedetails
